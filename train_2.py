...
for i in range(10):
    ...
    for j in tqdm(range(150), total=14500 ,desc = "Second Loop", unit='Iterations', unit_scale=True):
        ...
        for k in range(10):
            for l in range(4):
                for m in range(64):
                    Z3[k,l] = Z3[k,l] + weights_3[m,k]* A2[m,l]

        for k in range(10):
            for l in range(4):
                Z3[k,l] = Z3[k,l] + baises_3[k,:]
        ...
        for k in range(10):
            for l in range(4):
                Z3_back[k,l] = A3[k,l] - y_train[l,k]

        for k in range(64):
            for l in range(10):
                for m in range(4):
                    dloss_dweights_3[k,l] = dloss_dweights_3[k,l] + A2[k,m]* Z3_back[l,m]

        dloss_dbaises_3 = np.sum(Z3_back, axis = 1)
        dloss_dbaises_3 = np.reshape(dloss_dbaises_3,(10,1))
        ##########################################################
        dloss_dweights_3 =  dloss_dweights_3/batch_size
        dloss_dbaises_3 = dloss_dbaises_3/batch_size
        #########################################################
        ...
        for k in range(64):
            for l in range(4):
                for m in range(10):
                    dloss_dA2[k,l] = dloss_dA2[k,l] + weights_3[k,m]* Z3_back[m,l]
       ...
       for k in range(64):
        ...
        dloss_dbaises_2 = np.reshape(dloss_dbaises_2,(64,1))
        ##########################################################
        ...
        for k in range(64):
            for l in range(10):
                weights_3[k,l] = weights_3[k,l] - learning_rate*dloss_dweights_3[k,l]

        for k in range(10):
            baises_3[k,:] = baises_3[k,:] - learning_rate*dloss_dbaises_3[k,:]
